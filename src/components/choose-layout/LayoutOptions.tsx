...
interface LayoutOptionsProps {
  options: OptionsLayoutType[];
  label?: string;
  placeholder?: string;
  variant?: "primary" | "secondary";
  value?: string;
  onChange: (value: SelectedLayoutValue) => void;
  disabled?: boolean;
}
...
export default function LayoutOptions({
  ...
  const variants = {
    primary:
      "bg-black w-full rounded-lg md:px-4 px-2 py-1 md:py-2 text-xs md:text-sm",
    secondary: "border-gray-500 border focus:ring-gray-300",
  };

  const handleSelect = (value: SelectedLayoutValue, label: string) => {
    onChange(value);
    setSelected(label!);
    setIsOpen(false);
  };
  ...
  return (
    <div className="relative w-full" ref={ref}>
      ...
      <div
        onClick={handleOpen}
        className={clsx(
          "font-poppins flex items-center justify-between gap-3 rounded-md p-2 text-white capitalize transition-all duration-300 focus:ring-2 focus:outline-none",
      ...
      >
        {options.map((option, index) => (
          <li
            key={index}
            className={cn(
              "cursor-pointer rounded-md p-2 capitalize",
